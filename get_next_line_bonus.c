...
{
	static char	*read_content[OPEN_MAX];
	int			char_read;
	char		*buffer;
	...
	char_read = 1;
	while (char_read > 0 && found_line(read_content[fd]) == 0)
	{
		...
		buffer[char_read] = '\0';
		ft_cpy_content(&(read_content[fd]), buffer);
	}
	free(buffer);
	return (ft_transfer_line(&(read_content[fd]), line));
}
